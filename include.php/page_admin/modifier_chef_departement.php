...
if(isset($_GET['id']) && $_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    $sql = "SELECT * FROM `chefs_departement` WHERE `ID_chef_departement` = $id_chef_departement";
    $result = mysqli_query($conn, $sql);

    if(mysqli_num_rows($result) > 0) {
        $row = mysqli_fetch_assoc($result);

        // Affichage du formulaire de modification avec les données de l'chef_departement
?>
...
<!DOCTYPE html>
 <html lang="en">
        <head>
            ...
            <title>Modifier un chef_departement</title>
        ...
        <input type="hidden" name="id" value="<?php echo $id_chef_departement; ?>"> <!-- Champ caché pour transmettre l'ID de l'utilisateur -->
        ...
        $sql = "UPDATE `chefs_departement` SET `ID_utilisateur`='$ID_utilisateur',`ID_filière`='$ID_filiere'
               WHERE `ID_chef_departement`='$id'";
        if (mysqli_query($conn, $sql)) {
            echo "<script> alert('chef_departement modifié avec succès.')</script>";

            echo "<script> setTimeout(function(){ window.location.href = 'affchefs_Departement.php'; }, 10);</script>";
        } else {
            echo "<script> alert('Erreur lors de la modification de l\'chef_departement! Veuillez réessayer.')</script>";
            echo "Erreur lors de la modification de l'chef_departement : " . mysqli_error($conn);
        }
    } 
else {
    echo "<script> alert('ID d\'utilisateur non valide.')</script>";
