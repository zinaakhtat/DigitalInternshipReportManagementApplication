...
if(isset($_GET['id']) && $_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    $sql = "DELETE FROM `rapports_etudiants` WHERE `ID_rapport_etudiant` = $id_rapport";

    if (mysqli_query($conn, $sql)) {
        echo "<script> alert('rapport supprimé avec succès.')</script>";
        // Redirection vers la page des rapports après 2 secondes
        echo "<script> setTimeout(function(){ window.location.href = 'affRapports_Etudiant.php'; }, 10);</script>";
    } else {
