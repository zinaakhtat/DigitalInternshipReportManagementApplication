...
$sql = "SELECT ID_niveau, Nom_niveau FROM niveaux";
$result = $conn->query($sql);
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        $niveau_options .= '<option value="' . $row["ID_niveau"] . '">' . $row["Nom_niveau"] . '</option>';
    }
} else {
    $niveau_options = '<option value="" disabled selected hidden>Aucun niveau disponible</option>';
}
?>
...
if (isset($_POST['ajouter_etudiant'])) {
    ...
    if (!empty($_POST['filiere']) && !empty($_POST['ID_niveau'])) {
        $ID_filiere = $_POST['filiere'];
        $ID_niveau = $_POST['ID_niveau'];
        $ID_utilisateur = $_POST['ID_utilisateur']; // Récupérer l'ID de l'utilisateur depuis le formulaire

        // Insertion des données dans la table des étudiants
        $sql = "INSERT INTO etudiants (ID_utilisateur, ID_filière, ID_niveau) VALUES ('$ID_utilisateur', '$ID_filiere', '$ID_niveau')";
        if (mysqli_query($conn, $sql)) {
            echo "<script> alert('Étudiant ajouté avec succès.')</script>";
            header("Location:affetudiant.php");
...
<body>
    <h2>Veillez choisir la filiere et le niveau de l'etudiant</h2>
    ...
    <form id="etudiant_form" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
        ...
        <option value="" disabled selected hidden>Sélectionner un niveau</option>
            <?php echo $niveau_options; ?>
        </select>

        <input type="submit" value="Ajouter Étudiant" name="ajouter_etudiant">
