...
if(isset($_POST['ajouter']) && $_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(!empty($_POST['nom'])) {
        ...
        $sql = "INSERT INTO `roles`( `Nom_role`) 
                VALUES ('$nom')";
        if (mysqli_query($conn, $sql)) {
            echo "<script> alert('role ajouté avec succès.')</script>";
            // Redirect to the main page after 2 seconds
            echo "<script> setTimeout(function(){ window.location.href = 'affrole.php'; }, 10);</script>";
        } else {
            echo "<script> alert('Erreur lors de l\'ajout du role! Essayer une autre fois:')</script>";
            echo "Erreur lors de l'ajout du role: " . mysqli_error($conn);
        }
    } else {
