...
if (isset($_POST['ajouter_chef'])) {
    // Vérification si le formulaire a été soumis
    if (!empty($_POST['filiere'])) {
        $ID_filiere = $_POST['filiere'];

        // Insertion des données dans la table des étudiants
        $sql = "INSERT INTO chefs_departement (ID_utilisateur, ID_filière) VALUES ('$ID_utilisateur', '$ID_filiere')";
        if (mysqli_query($conn, $sql)) {
            echo "<script> alert('chef_departement ajouté avec succès.')</script>";
            header("Location:affchefs_Departement.php");

        } else {
            echo "<script> alert('Erreur lors de l\'ajout de l\'chef_departement! Essayer une autre fois.')</script>";
            echo "Erreur lors de l'ajout de l'chef_departement : " . mysqli_error($conn);
        }
    } else {
...
<body>
    ...
    <form id="etudiant_form" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
        ...
        <input type="submit" value="Ajouter chef" name="ajouter_chef">
    </form>
